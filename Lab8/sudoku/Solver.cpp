...
Solver::Solver(const Board &b) { board_ = b; }
...
std::vector<Value> Solver::findPossibleValues(int row, int col) const {
  ...
  allPossibleValues =
      removeValuesWhichExistInBlock(row, col, allPossibleValues);
...
std::vector<Value>
Solver::removeValuesWhichExistInColumn(int col,
...
std::vector<Value>
Solver::removeValuesWhichExistInRow(int row, std::vector<Value> values) const {
  for (int i = 0; i < BOARD_SIZE; i++) {
...
std::vector<Value>
Solver::removeValuesWhichExistInBlock(int row, int col,
                                      std::vector<Value> values) const {
...
Board Solver::checkPossibleSolutions(int row, int col,
