...
Validator::Validator(Board b) { board_ = b; }
...
bool Validator::isValid() {
  ...
  return checkResults(blockValidationResults, columnValidationResults,
...
bool Validator::checkColumn(std::vector<Value> col) {
  std::set<int> unique(col.begin(),col.end());

  return unique.size() == 9;
}

std::vector<Value> Validator::getRow(const Board &b, const int &index) { return b[index]; }
...
bool Validator::checkResults(const std::vector<bool> &blockValidationResults,
                             const std::vector<bool> &columnValidationResults,
                             const std::vector<bool> &rowValidationResults) {
